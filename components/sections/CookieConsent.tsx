...
export default function CookieConsent() {
  ...
  return (
    <>
      {/* Cookie Banner */}
      <div 
        className={`fixed right-6 bottom-6 max-w-[420px] w-[calc(100%-48px)] p-5 rounded-2xl bg-bg-secondary/90 border border-[#00d4ff]/15 backdrop-blur-xl shadow-2xl z-[9999] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          showBanner ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0 pointer-events-none'
        }`}
      >
        <div className="text-xs md:text-sm text-text-secondary leading-relaxed mb-4">
        ...
        <div className="flex gap-3 justify-end text-xs font-bold">
          <button 
            ...
            className="px-4 py-2 rounded-full border border-white/10 hover:border-white/20 text-text-secondary hover:text-text-primary transition-colors"
          >
            Reject
          </button>
          <button 
            onClick={() => handleConsent('accepted')} 
            className="px-5 py-2 rounded-full bg-gradient-to-r from-[#00d4ff] to-[#7c3aed] text-text-primary shadow-[0_4px_12px_rgba(0,212,255,0.25)] hover:shadow-[0_4px_20px_rgba(0,212,255,0.4)] transition-all hover:scale-102"
          >
            Accept
          </button>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[10000] flex items-center justify-center p-6 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-bg-secondary border border-[#00d4ff]/15 rounded-2xl max-w-[450px] w-full p-8 shadow-2xl text-left">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.01] border border-white/5">
                <div>
                  <div className="text-xs md:text-sm font-semibold text-text-primary">Essential Cookies</div>
                  <div className="text-[10px] text-text-muted mt-0.5">Required for security and CRM systems.</div>
                </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.01] border border-white/5">
                <div>
                  <div className="text-xs md:text-sm font-semibold text-text-primary">Analytics Cookies</div>
                  <div className="text-[10px] text-text-muted mt-0.5">Help us analyze website visitor traffic.</div>
                </div>
                <input 
                  type="checkbox" 
            ...
            <div className="flex justify-end gap-3 text-xs font-bold">
              <button 
                ...
                className="px-4 py-2.5 rounded-full border border-white/10 text-text-secondary hover:text-text-primary transition-colors"
              >
                Cancel
              </button>
              <button 
                onClick={savePreferences} 
                className="px-6 py-2.5 rounded-full bg-gradient-to-r from-[#00d4ff] to-[#7c3aed] text-text-primary shadow-[0_4px_12px_rgba(0,212,255,0.25)] transition-all hover:scale-102"
              >
                Save Settings
              </button>
            </div>
