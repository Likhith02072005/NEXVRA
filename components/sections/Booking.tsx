...
export default function Booking() {
  ...
  return (
    <section id="booking" className="py-24 bg-[#050508] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col items-start">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white mt-6 mb-6 leading-tight font-display">
              Book Your Free <br />
            ...
            <ul className="space-y-6">
              {[
                ...
                <li key={idx} className="flex gap-4 items-start">
                  ...
                  <span className="text-slate-300 text-sm font-medium pt-1.5">{b.title}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="p-8 rounded-2xl bg-white/[0.01] border border-white/5 relative overflow-hidden glass-panel" data-cursor-label="Schedule">
              ...
              {!success ? (
                <div>
                  ...
                  <p className="text-slate-400 text-xs md:text-sm mb-6">Pick a date and time that works for you.</p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex flex-col">
                        ...
                        <input
                          type="text"
                          className="w-full bg-[#0a0a0f] border border-white/5 focus:border-[#00d4ff]/30 text-white rounded-xl px-4 py-3 text-xs md:text-sm outline-none transition-colors"
                      ...
                      <div className="flex flex-col">
                        <label className="text-xs text-slate-400 font-semibold mb-2">Email *</label>
                        <input
                          type="email"
                          className="w-full bg-[#0a0a0f] border border-white/5 focus:border-[#00d4ff]/30 text-white rounded-xl px-4 py-3 text-xs md:text-sm outline-none transition-colors"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex flex-col">
                        <label className="text-xs text-slate-400 font-semibold mb-2">Phone *</label>
                        <input
                          type="tel"
                          className="w-full bg-[#0a0a0f] border border-white/5 focus:border-[#00d4ff]/30 text-white rounded-xl px-4 py-3 text-xs md:text-sm outline-none transition-colors"
                      ...
                      <div className="flex flex-col">
                        <label className="text-xs text-slate-400 font-semibold mb-2">Business Type *</label>
                        <select
                          className="w-full bg-[#0a0a0f] border border-white/5 focus:border-[#00d4ff]/30 text-white rounded-xl px-4 py-3 text-xs md:text-sm outline-none transition-colors"
                    ...
                    <div className="flex flex-col">
                      <label className="text-xs text-slate-400 font-semibold mb-3">Preferred Date *</label>
                      <div className="bg-[#0a0a0f] border border-white/5 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-4">
                          <span className="text-xs font-bold text-white uppercase tracking-wider">{monthName} {year}</span>
                          <div className="flex gap-1">
                            <button
                              type="button"
                              onClick={handlePrevMonth}
                              className="w-6 h-6 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white text-xs font-bold transition-all"
                            >
                              ‹
                            </button>
                            <button
                            ...
                            return (
                              <button
                                ...
                                className={`h-8 rounded-lg text-xs font-semibold flex items-center justify-center transition-all ${
                                  selected
                                    ? 'bg-gradient-to-r from-[#00d4ff] to-[#7c3aed] text-white font-extrabold shadow-[0_4px_10px_rgba(0,212,255,0.25)]'
                                    : past
                                    ? 'text-slate-700 cursor-not-allowed'
                                    : 'text-slate-300 bg-white/[0.02] hover:bg-white/[0.08]'
                                }`}
                              >
                                {day}
                              </button>
                    ...
                    <div className="flex flex-col">
                      <label className="text-xs text-slate-400 font-semibold mb-3">Preferred Time *</label>
                          ...
                          <button
                            ...
                            className={`py-2 rounded-xl text-center text-xs font-semibold transition-all ${
                              selectedTimeSlot === slot
                                ? 'bg-gradient-to-r from-[#00d4ff] to-[#7c3aed] text-white shadow-[0_4px_10px_rgba(0,212,255,0.25)]'
                                : 'bg-[#0a0a0f] border border-white/5 text-slate-300 hover:bg-white/[0.08]'
                            }`}
                          >
                            {slot}
                          </button>
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="w-full text-center py-4 rounded-full bg-gradient-to-r from-[#00d4ff] to-[#7c3aed] text-white font-extrabold text-xs md:text-sm tracking-wider shadow-[0_8px_24px_rgba(0,212,255,0.3)] hover:shadow-[0_8px_35px_rgba(0,212,255,0.5)] transition-all hover:scale-102"
                    >
